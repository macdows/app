...
const Protections: FunctionComponent<Props> = ({ application }) => {
  ...
  return (
    <PreferencesGroup>
      <PreferencesSegment>
        <Title>{c('Title').t`Protections`}</Title>
        {protectionsDisabledUntil ? (
          <Text className="text-info">{c('Info').t`Unprotected access expires at ${protectionsDisabledUntil}.`}</Text>
        ) : (
          <Text className="text-info">{c('Info').t`Protections are enabled.`}</Text>
        )}
        <Text className="mt-2">
          {c('Info')
        ...
        {protectionsDisabledUntil && (
          <Button className="mt-3" primary label={c('Action').t`End Unprotected Access`} onClick={enableProtections} />
        )}
      </PreferencesSegment>
    </PreferencesGroup>
  )
